...
def _fit_matrices(Y, X, method: str, **kwargs):
    ...
    def predict_wrapper(newdata=None, *args, **kwargs):
        """
        Make predictions on new data, optionally with confidence or prediction intervals.
        ...
        *args, **kwargs
            Additional prediction options passed to ``ravix.predict`` such as
            ``interval``, ``level``, and ``alpha``.
        
        Returns
        -------
        array-like or DataFrame
            Predicted values, or a three-column interval DataFrame when an
        ...
        >>> pi = model.predict(new_data, interval="prediction")
...
def _build_selection_groups(actual_predictors, categorical_levels, group_categorical=False):
    """Build selectable predictor groups for stepwise and best-subsets selection.

    With ``group_categorical=False`` each design-matrix column is its own
    candidate, preserving Ravix's historical behavior. With
    ``group_categorical=True``, the dummy columns generated by each categorical
    main-effect predictor are treated as one candidate and enter/leave together.
    """
    if not group_categorical:
        return {predictor: [predictor] for predictor in actual_predictors}

    predictor_set = set(actual_predictors)
    groups = {}
    consumed = set()

    for var, levels in categorical_levels.items():
        dummy_cols = [f"{var}_{level}" for level in levels[1:]]
        if dummy_cols and all(col in predictor_set for col in dummy_cols):
            groups[var] = dummy_cols
            consumed.update(dummy_cols)

    for predictor in actual_predictors:
        if predictor not in consumed:
            groups[predictor] = [predictor]
...
def _attach_formula_if_reconstructible(model, response_term, categorical_levels):
    ...
    subset or NONE are. With the default ungrouped selection, bsr/stepwise can
    produce models with only *some* of a categorical variable's levels
    included; group_categorical=True prevents that for categorical main effects.
    Interaction
